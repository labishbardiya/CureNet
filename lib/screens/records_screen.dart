...
class _RecordsScreenState extends State<RecordsScreen> {
  ...
  List<Map<String, dynamic>> get _filteredRecords {
    ...
    // Convert saved OCR records to the display format
    for (var r in _savedRecords) {
      final uiData = r['uiData'] ?? {};
      final docType = uiData['document_type'] ?? 'unknown';
      final summary = uiData['summary'] ?? {};
      
      String title = docType == 'prescription' ? "Scanned Prescription" : 
                    docType == 'lab_report' ? "Scanned Lab Report" : "Scanned Document";
      
      all.add({
        "isReal": true,
        "rawData": r,
        "title": title,
        "date": summary['date'] ?? r['savedAt']?.substring(0, 10) ?? "Recent",
        "doctor": summary['doctor'] ?? summary['facility'] ?? "Unknown",
        "type": docType == 'prescription' ? Icons.medication : 
                docType == 'lab_report' ? Icons.science : Icons.document_scanner,
        "color": docType == 'prescription' ? "#E07B39" : "#00A3A3",
        "docType": docType
      });
    }
    
    // Add mocks
    for (var m in mockRecords) {
      all.add({
        ...m,
        "isReal": false,
        "docType": m['type'] == Icons.medication ? 'prescription' : 
                   m['type'] == Icons.science ? 'lab_report' : 'other'
      });
    }

    // Filter by tab
    if (activeTab == 2) { // Prescriptions
      return all.where((r) => r['docType'] == 'prescription').toList();
    } else if (activeTab == 3) { // Labs
      return all.where((r) => r['docType'] == 'lab_report').toList();
    } else if (activeTab == 4) { // Reports
      return all.where((r) => r['docType'] == 'other').toList();
    }
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  onTap: () {
                    if (record['isReal'] == true) {
                      final rawData = record['rawData'];
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => ScanResultScreen(
                            uiData: rawData['uiData'] ?? {},
                            fhirBundle: rawData['fhirBundle'] ?? {},
                            abdmContext: rawData['abdmContext'] ?? {},
                          ),
                        ),
                      );
                    } else {
